...
# 1. 데이터 준비
data_directory = 'CSV/일봉'  
company_name_to_find = 'SK하이닉스'

window_size = 30
future_days = 5

# 지정된 폴더에서 회사 이름으로 끝나는 CSV 파일 탐색
csv_filepath = None
try:
    for filename in os.listdir(data_directory):
        if filename.endswith(f"{company_name_to_find}.csv"):
            csv_filepath = os.path.join(data_directory, filename)
            break # 파일을 찾았으면 반복 중단
except FileNotFoundError:
    print(f"\n[Error] The directory '{data_directory}' was not found. Please create it and place your CSV files inside.")
    exit()

if csv_filepath is None:
    print(f"\n[Error] Could not find a CSV file for '{company_name_to_find}' in the '{data_directory}' folder.")
    exit()

company_name = company_name_to_find # 찾은 회사 이름을 그래프 제목 등에 사용
