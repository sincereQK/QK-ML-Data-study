...
try:
    ...
    company_name = company_info.info.get('longName', ticker) # longName이 없으면 ticker 코드를 사용
except Exception as e:
    print(f"Could not fetch company name for {ticker}: {e}")
    company_name = ticker # 에러 발생 시 ticker 코드를 사용
...
if len(prices) <= window_size:
    ...
    print(f"Data for '{company_name} ({ticker})' is ready. (Length: {len(prices)} days)")
